...
from config import (
    EMA_MAX_SL_PCT,
    EMA_MAX_TARGET_PROFIT_PCT,
    EMA_MIN_TARGET_PROFIT_PCT,
    EMA_RISK_REWARD,
    MAX_SL_PCT_PLAYBOOK,
...
EMA_TRAIL_NOTE = (
    "9/15 EMA crossover · SL = prior 5m candle extreme (capped). "
    "Book 70% at T1 · trail runner · square off by 3:30 PM IST."
)
...
def levels_ema_crossover(
    entry: float,
    structural_stop: float,
    side: str,
) -> TradeLevels | None:
    """
    Low-risk EMA setup: tight SL cap, target 2–3% with minimum 1:2 R:R.
    Structural stop = previous candle low (BUY) or high (SELL).
    """
    max_sl_frac = EMA_MAX_SL_PCT / 100.0
    min_profit_frac = EMA_MIN_TARGET_PROFIT_PCT / 100.0
    max_profit_frac = EMA_MAX_TARGET_PROFIT_PCT / 100.0
    min_rr = EMA_RISK_REWARD
    entry = _round_price(entry)
    if entry <= 0:
        return None
    ...
    if side == "BUY":
        ...
        if risk < 0.01:
            return None
        t1 = entry + risk * 1.0
        t2_rr = entry + risk * min_rr
        best = max(t2_rr, entry * (1 + min_profit_frac))
        best = min(best, entry * (1 + max_profit_frac))
        rr = (best - entry) / risk if risk > 0 else min_rr
        return TradeLevels(
            entry=entry,
            stop_loss=_round_price(structural_stop),
            target_1=_round_price(t1),
            target_2=_round_price(t2_rr),
            best_target=_round_price(best),
            rr_best=round(rr, 2),
            trailing_note=EMA_TRAIL_NOTE,
            risk=_round_price(risk),
            reward_1=_round_price(t1 - entry),
            reward_2=_round_price(best - entry),
        )
    ...
    t1 = entry - risk * 1.0
    t2_rr = entry - risk * min_rr
    best = min(t2_rr, entry * (1 - min_profit_frac))
    best = max(best, entry * (1 - max_profit_frac))
    rr = (entry - best) / risk if risk > 0 else min_rr
    return TradeLevels(
        entry=entry,
        stop_loss=_round_price(structural_stop),
        target_1=_round_price(t1),
        ...
        risk=_round_price(risk),
        reward_1=_round_price(entry - t1),
        reward_2=_round_price(entry - best),
    )
